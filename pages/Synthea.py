...
st.set_page_config(page_title="Propensity Score Calculator - Synthea", layout="centered")
st.title("Propensity Score Calculator - Synthea")
st.warning(
    "These propensity scores are based on synthetic data from the Synthea Dataset. "
    "This dataset covers generalized diagnoses but is limited in scope."
)
st.markdown(
    "**Synthea** is an open-source, synthetic patient generator that models the medical history "
    "of synthetic patients. The resulting data is free from cost, privacy, and security restrictions.",
...
@st.cache_resource
def get_trained_models(patients_csv, medications_csv, diagnosis_count):
...
tier_models, df, meds_seen = get_trained_models(patients_csv, medications_csv, diagnosis_count=50)
